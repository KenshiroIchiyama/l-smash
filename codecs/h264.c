...
)
{
    if( !info )
        return;
    /* TODO: Check why these eliminators are set to NULL. */
...
)
{
    ...
    lsmash_list_init( info->sps_list,   isom_remove_dcr_ps );
    lsmash_list_init( info->pps_list,   isom_remove_dcr_ps );
    lsmash_list_init( info->slice_list, isom_remove_dcr_ps );
    return 0;
}
